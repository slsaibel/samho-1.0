 ...
 * 
 */
public class IFrmPesqMotivosAgendas extends JInternalFrame {

    // Declaração de atributos
    private IFrmManMotivoAgenda iFrmManMotivoAgenda;
    private MotivosAgendas motivoAgenda;
    private JTextField jTextFieldCodigo;
    private JTextField jTextFieldDescricao;
    private ArrayList<DadosDAO> dadosWhere;

    // Construtor
    public IFrmPesqMotivosAgendas(IFrmManMotivoAgenda iFrmManMotivoAgenda) 
            throws Exception {
        inicializar(iFrmManMotivoAgenda);
    }
    ...
    private void inicializar(IFrmManMotivoAgenda iFrmManMotivoAgenda) {
        initComponents();

        tbResultado.getTableHeader().setReorderingAllowed(false);
        this.iFrmManMotivoAgenda = iFrmManMotivoAgenda;
        motivoAgenda = iFrmManMotivoAgenda.getMotivoAgenda();
        dadosWhere = new ArrayList<>();
        dadosWhere.addAll(motivoAgenda.getObjetoDAO().getCondicaoWhere());

        Util.atualizar(motivoAgenda, tbResultado);
     ...
     */
      public void verificarPermissoes (){
        boolean[] acoes = Util.getAcoesUsuario(Principal.MOTIVOS_AGENDAS);
    ...
    private void reescreverObjeto() {
        try {
            motivoAgenda.setIdMotivoAgenda(Long.parseUnsignedLong(
                    tbResultado.getValueAt(
                            tbResultado.getSelectedRow(), 0).toString()));
            motivoAgenda.setDescricao(tbResultado.getValueAt(
                    tbResultado.getSelectedRow(), 1).toString());
            motivoAgenda.setObservacoes(tbResultado.getValueAt(
                    tbResultado.getSelectedRow(), 2).toString());
            // Desabilita os itens alterados por padrão
            motivoAgenda.setAtivo(false);

            motivoAgenda.adicionarCampos();
    ...
    private void efetuarPesquisa() {
        motivoAgenda.removerWhere();

        if (!txtCodigo.getText().equals("")) {
            Util.verificarPesquisarCampoID(motivoAgenda, txtCodigo);
        }

        if (!txtDescricao.getText().equals("")) {
            Util.verificarPesquisarCampoDescricao(motivoAgenda, txtDescricao);
        }
        
        if (chkAtivo.isSelected()) {
            Util.verificarPesquisarObjetoAtivo(motivoAgenda, chkAtivo);
        }

        motivoAgenda.adicionarWhere(dadosWhere);
    ...
    private void initComponents() {
        ...
        setMaximizable(true);
        setTitle("Consultar Motivos Agendas");
    ...
    private void btnIncluirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnIncluirActionPerformed
        try {
            iFrmManMotivoAgenda.carregarObjetoIFrmManutencao(new MotivosAgendas());

            Util.incluir(this, iFrmManMotivoAgenda);
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this.getParent(),
    ...
    private void btnAlterarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAlterarActionPerformed
        try {
            reescreverObjeto();
            iFrmManMotivoAgenda.carregarObjetoIFrmManutencao(motivoAgenda);

            Util.alterar(this, iFrmManMotivoAgenda);
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this.getParent(),
    ...
    private void btnExcluirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnExcluirActionPerformed
        Util.excluir(motivoAgenda, tbResultado, this);
            ...
            try {
                ...
                if ((jTextFieldCodigo == null) && (jTextFieldDescricao == null)) {
                    ...
                    jTextFieldCodigo.setText(String.valueOf(
                            motivoAgenda.getIdMotivoAgenda()));
                    jTextFieldDescricao.setText(motivoAgenda.getDescricao());
    ...
    /**
     * @return the motivoAgenda
     */
    public MotivosAgendas getMotivoAgenda() {
        return motivoAgenda;
    }

    /**
     * @param motivoAgenda the motivoAgenda to set
     */
    public void setMotivoAgenda(MotivosAgendas motivoAgenda) {
