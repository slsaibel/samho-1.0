 ...
 * 
 */
public class IFrmPesqAcoes extends JInternalFrame {

    // Declaração de atributos
    private IFrmManAcao iFrmManAcao;
    private Acoes acao;
    ...
    // Construtor
    public IFrmPesqAcoes(IFrmManAcao iFrmManAcao) throws Exception {
        inicializar(iFrmManAcao);
    }

    public IFrmPesqAcoes(IFrmManAcao iFrmManAcao, JTextField jTextFieldCodigo,
            JTextField jTextFieldDescricao) throws Exception {
        inicializar(iFrmManAcao);
    ...
    private void inicializar(IFrmManAcao iFrmManAcao) {
        initComponents();

        tbResultado.getTableHeader().setReorderingAllowed(false);
        this.iFrmManAcao = iFrmManAcao;
        acao = iFrmManAcao.getAcao();
        dadosWhere = new ArrayList<>();
        dadosWhere.addAll(acao.getObjetoDAO().getCondicaoWhere());

        Util.atualizar(acao, tbResultado);
    ...
    private void reescreverObjeto() {
        try {
            acao.setIdAcao(Long.parseUnsignedLong(
                    tbResultado.getValueAt(
                            tbResultado.getSelectedRow(), 0).toString()));
            acao.setDescricao(tbResultado.getValueAt(
                    tbResultado.getSelectedRow(), 1).toString());

            acao.adicionarCampos();
    ...
    private void efetuarPesquisa() {
        ...
        if (!txtCodigo.getText().equals("")) {
            Util.verificarPesquisarCampoID(acao, txtCodigo);
        }

        if (!txtDescricao.getText().equals("")) {
            Util.verificarPesquisarCampoDescricao(acao, txtDescricao);
        }

        acao.adicionarWhere(dadosWhere);
    ...
    private void btnIncluirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnIncluirActionPerformed
        try {
            iFrmManAcao.carregarObjetoIFrmManutencao(new Acoes());

            Util.incluir(this, iFrmManAcao);
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this.getParent(),
    ...
    private void btnAlterarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAlterarActionPerformed
        try {
            reescreverObjeto();
            iFrmManAcao.carregarObjetoIFrmManutencao(acao);

            Util.alterar(this, iFrmManAcao);
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this.getParent(),
    ...
    private void btnExcluirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnExcluirActionPerformed
        Util.excluir(acao, tbResultado, this);
        ...
        if ((btnAlterar.isEnabled()) && (evt.getClickCount() == 2)) {
            try {
                ...
                if ((jTextFieldCodigo == null) && (jTextFieldDescricao == null)) {
                    ...
                    jTextFieldCodigo.setText(String.valueOf(acao.getIdAcao()));
                    jTextFieldDescricao.setText(acao.getDescricao());
