 ...
 * 
 */
public class IFrmPesqSintomasApresentados extends JInternalFrame {

    // Declaração de atributos
    private IFrmManSintomaApresentado iFrmManSintomaApresentado;
    private SintomasApresentados sintomaApresentado;
    private JTextField jTextFieldCodigo;
    private JTextField jTextFieldDescricao;
    private ArrayList<DadosDAO> dadosWhere;

    // Construtor
    public IFrmPesqSintomasApresentados(
            IFrmManSintomaApresentado iFrmManSintomaApresentado) throws Exception {
        inicializar(iFrmManSintomaApresentado);
    }

    public IFrmPesqSintomasApresentados(
            IFrmManSintomaApresentado iFrmManSintomaApresentado,
    ...
    private void inicializar(IFrmManSintomaApresentado iFrmManSintomaApresentado) {
        initComponents();

        tbResultado.getTableHeader().setReorderingAllowed(false);
        this.iFrmManSintomaApresentado = iFrmManSintomaApresentado;
        sintomaApresentado = iFrmManSintomaApresentado.getSintomaApresentado();
        dadosWhere = new ArrayList<>();
        dadosWhere.addAll(sintomaApresentado.getObjetoDAO().getCondicaoWhere());

        Util.atualizar(sintomaApresentado, tbResultado);
    ...
    private void reescreverObjeto() {
        try {
            sintomaApresentado.setIdSintomaApresentado(Long.parseUnsignedLong(
                    tbResultado.getValueAt(
                            tbResultado.getSelectedRow(), 0).toString()));
            sintomaApresentado.setDescricao(tbResultado.getValueAt(
                    tbResultado.getSelectedRow(), 1).toString());
            sintomaApresentado.setObservacoes(tbResultado.getValueAt(
                    tbResultado.getSelectedRow(), 2).toString());
            // Desabilita os itens alterados por padrão
            sintomaApresentado.setAtivo(false);

            sintomaApresentado.adicionarCampos();
    ...
    private void efetuarPesquisa() {
        sintomaApresentado.removerWhere();

        if (!txtCodigo.getText().equals("")) {
            Util.verificarPesquisarCampoID(sintomaApresentado, txtCodigo);
        }

        if (!txtDescricao.getText().equals("")) {
            Util.verificarPesquisarCampoDescricao(sintomaApresentado, txtDescricao);
        }
        
        if (chkAtivo.isSelected()) {
            Util.verificarPesquisarObjetoAtivo(sintomaApresentado, chkAtivo);
        }

        sintomaApresentado.adicionarWhere(dadosWhere);
    ...
    private void initComponents() {
        ...
        setMaximizable(true);
        setTitle("Consultar Sintomas Apresentados");
    ...
    private void btnIncluirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnIncluirActionPerformed
        try {
            iFrmManSintomaApresentado.carregarObjetoIFrmManutencao(
                    new SintomasApresentados());

            Util.incluir(this, iFrmManSintomaApresentado);
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this.getParent(),
    ...
    private void btnAlterarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAlterarActionPerformed
        try {
            ...
            Util.alterar(this, iFrmManSintomaApresentado);
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this.getParent(),
    ...
    private void btnExcluirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnExcluirActionPerformed
        Util.excluir(sintomaApresentado, tbResultado, this);
        ...
        if ((btnAlterar.isEnabled()) && (evt.getClickCount() == 2)) {
            try {
                ...
                if ((jTextFieldCodigo == null) && (jTextFieldDescricao == null)) {
                    iFrmManSintomaApresentado.carregarObjetoIFrmManutencao(sintomaApresentado);

                    Util.alterar(this, iFrmManSintomaApresentado);
                } else {
                    jTextFieldCodigo.setText(String.valueOf(
                            sintomaApresentado.getIdSintomaApresentado()));
                    jTextFieldDescricao.setText(sintomaApresentado.getDescricao());
    ...
    /**
     * @return the sintomaApresentado
     */
    public SintomasApresentados getSintomaApresentado() {
        return sintomaApresentado;
    }

    /**
     * @param sintomaApresentado the sintomaApresentado to set
     */
    public void setSintomaApresentado(SintomasApresentados sintomaApresentado) {
        this.sintomaApresentado = sintomaApresentado;
    }
}
