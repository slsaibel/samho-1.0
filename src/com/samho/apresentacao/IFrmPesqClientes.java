 ...
 *
 * @version  revision 001.20140422 date 22/04/2014 author Saibel, Sérgio Luis reason
 * Conseguir instanciar um objeto do tipo FrmPesquisa com os atributos e métodos
 * necessários para efetuar pesquisas.
 */
public class IFrmPesqClientes extends javax.swing.JInternalFrame {

    // Declaração de atributos
    private IFrmManCliente iFrmManCliente;
    ...
    // Construtor
    public IFrmPesqClientes(IFrmManCliente iFrmManCliente) throws Exception {
        inicializar(iFrmManCliente);
    }

    public IFrmPesqClientes(IFrmManCliente iFrmManCliente,
    ...
    private void inicializar(IFrmManCliente iFrmManCliente) {
        initComponents();

        this.iFrmManCliente = iFrmManCliente;
        cliente = iFrmManCliente.getCliente();
        dadosWhere = new ArrayList<>();
        dadosWhere.addAll(cliente.getObjetoDAO().getCondicaoWhere());

        Util.atualizar(cliente, tbResultado);
    ...
    private void reescreverObjeto() {
        try {
            cliente.setIdCliente(Long.parseUnsignedLong(tbResultado.getValueAt(
                    tbResultado.getSelectedRow(), 0).toString()));
            cliente.setCodPessoa(Long.parseUnsignedLong(tbResultado.getValueAt(
                    tbResultado.getSelectedRow(), 1).toString()));
            // Reserva-se a próxima posição para a descrição do campo acima.
            cliente.setCodSituacao(Long.parseUnsignedLong(tbResultado.getValueAt(
            ...
            cliente.setDataDeCadastro(Formatacao.ajustaData(
                    tbResultado.getValueAt(
                            ...
                            Formatacao.DATA_AMD));
            cliente.setObservacoes(String.valueOf(tbResultado.getValueAt(
                    tbResultado.getSelectedRow(), 6)));
    ...
    private void efetuarPesquisa() {
        cliente.removerWhere();

        if (!txtCodigo.getText().equals("")) {
            Util.verificarPesquisarCampoID(cliente, txtCodigo);
        }

        if (!txtDescricao.getText().equals("")) {
            Util.verificarPesquisarCampoDescricao(cliente, txtDescricao);
        }

        cliente.adicionarWhere(dadosWhere);
    ...
    private void btnIncluirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnIncluirActionPerformed
        try {
            iFrmManCliente.carregarObjetoIFrmManutencao(new Clientes());

            Util.incluir(this, iFrmManCliente);
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this.getParent(),
    ...
    private void btnAlterarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAlterarActionPerformed
        try {
            reescreverObjeto();
            iFrmManCliente.carregarObjetoIFrmManutencao(cliente);

            Util.alterar(this, iFrmManCliente);
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this.getParent(),
    ...
    private void btnExcluirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnExcluirActionPerformed
        Util.excluir(cliente, tbResultado, this);
