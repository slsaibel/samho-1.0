 ...
 * 
 */
public class IFrmPesqEstados extends JInternalFrame {

    // Declaração de atributos
    private IFrmManEstado iFrmManEstado;
    ...
    // Construtor
    public IFrmPesqEstados(IFrmManEstado iFrmManEstado) throws Exception {
        inicializar(iFrmManEstado);
    }

    public IFrmPesqEstados(IFrmManEstado iFrmManEstado,
    ...
    private void inicializar(IFrmManEstado iFrmManEstado) {
        initComponents();

        this.iFrmManEstado = iFrmManEstado;
        estado = iFrmManEstado.getEstado();
        dadosWhere = new ArrayList<>();
        dadosWhere.addAll(estado.getObjetoDAO().getCondicaoWhere());

        Util.atualizar(estado, tbResultado);
    ...
    public void verificarPermissoes (){
        boolean[] acoes = Util.getAcoesUsuario(Principal.ESTADOS);
    ...
    private void reescreverObjeto() {
        try {
            estado.setIdEstado(Long.parseUnsignedLong(tbResultado.getValueAt(
                    tbResultado.getSelectedRow(), 0).toString()));
            estado.setCodPais(Long.parseUnsignedLong(tbResultado.getValueAt(
                    tbResultado.getSelectedRow(), 1).toString()));
            // Reserva-se a próxima posição para a descrição do campo acima.
            estado.setCodigoIBGE(Integer.parseInt(tbResultado.getValueAt(
                    tbResultado.getSelectedRow(), 3).toString()));
            estado.setSigla(tbResultado.getValueAt(
                    tbResultado.getSelectedRow(), 4).toString());
            estado.setDescricao(tbResultado.getValueAt(
    ...
    private void efetuarPesquisa() {
        ...
        if (!txtCodigo.getText().equals("")) {
            Util.verificarPesquisarCampoID(estado, txtCodigo);
        }

        if (!txtDescricao.getText().equals("")) {
            Util.verificarPesquisarCampoDescricao(estado, txtDescricao);
        }

        estado.adicionarWhere(dadosWhere);
    ...
    private void btnIncluirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnIncluirActionPerformed
        try {
            iFrmManEstado.carregarObjetoIFrmManutencao(new Estados());

            Util.incluir(this, iFrmManEstado);
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this.getParent(),
    ...
    private void btnAlterarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAlterarActionPerformed
        try {
            reescreverObjeto();
            iFrmManEstado.carregarObjetoIFrmManutencao(estado);

            Util.alterar(this, iFrmManEstado);
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this.getParent(),
    ...
    private void btnExcluirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnExcluirActionPerformed
        Util.excluir(estado, tbResultado, this);
        ...
        if ((btnAlterar.isEnabled()) && (evt.getClickCount() == 2)) {
            try {
                ...
                if ((jTextFieldCodigo == null) && (jTextFieldDescricao == null)) {
                    ...
                    jTextFieldCodigo.setText(String.valueOf(estado.getIdEstado()));
