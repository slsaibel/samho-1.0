 ...
 * 
 */
public class IFrmPesqTiposPlanosSaude extends JInternalFrame {

    // Declaração de atributos
    private IFrmManTipoPlanoSaude iFrmManTipoPlanoSaude;
    ...
    // Construtor
    public IFrmPesqTiposPlanosSaude(IFrmManTipoPlanoSaude iFrmManTipoPlanoSaude)
            throws Exception {
        inicializar(iFrmManTipoPlanoSaude);
    }
    ...
    private void inicializar(IFrmManTipoPlanoSaude iFrmManTipoPlanoSaude) {
        initComponents();

        tbResultado.getTableHeader().setReorderingAllowed(false);
        this.iFrmManTipoPlanoSaude = iFrmManTipoPlanoSaude;
        tipoPlanoSaude = iFrmManTipoPlanoSaude.getTipoPlanoSaude();
        dadosWhere = new ArrayList<>();
        dadosWhere.addAll(tipoPlanoSaude.getObjetoDAO().getCondicaoWhere());

        Util.atualizar(tipoPlanoSaude, tbResultado);
    ...
    private void reescreverObjeto() {
        try {
            ...
            tipoPlanoSaude.setDescricao(tbResultado.getValueAt(
                    tbResultado.getSelectedRow(), 3).toString());
            tipoPlanoSaude.setVlrDiferenca(Double.parseDouble(
                    tbResultado.getValueAt(
            ...
            // Desabilita os itesn alterados por padrão
            tipoPlanoSaude.setAtivo(false);
    ...
    private void efetuarPesquisa() {
        tipoPlanoSaude.removerWhere();

        if (!txtCodigo.getText().equals("")) {
            Util.verificarPesquisarCampoID(tipoPlanoSaude, txtCodigo);
        }

        if (!txtDescricao.getText().equals("")) {
            Util.verificarPesquisarCampoDescricao(tipoPlanoSaude, txtDescricao);
        }
        
        if (chkAtivo.isSelected()) {
            Util.verificarPesquisarObjetoAtivo(tipoPlanoSaude, chkAtivo);
        }

        tipoPlanoSaude.adicionarWhere(dadosWhere);
    ...
    private void initComponents() {
        ...
        setMaximizable(true);
        setTitle("Consultar Tipos de Planos de Saúde");
    ...
    private void btnIncluirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnIncluirActionPerformed
        try {
            iFrmManTipoPlanoSaude.carregarObjetoIFrmManutencao(
                    new TiposPlanosSaude());

            Util.incluir(this, iFrmManTipoPlanoSaude);
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this.getParent(),
    ...
    private void btnAlterarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAlterarActionPerformed
        try {
            reescreverObjeto();
            iFrmManTipoPlanoSaude.carregarObjetoIFrmManutencao(tipoPlanoSaude);

            Util.alterar(this, iFrmManTipoPlanoSaude);
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this.getParent(),
    ...
    private void btnExcluirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnExcluirActionPerformed
        Util.excluir(tipoPlanoSaude, tbResultado, this);
        ...
        if ((btnAlterar.isEnabled()) && (evt.getClickCount() == 2)) {          
            try {
                ...
                if ((jTextFieldCodigo == null) && (jTextFieldDescricao == null)) {
                    ...
                    jTextFieldDescricao.setText(tipoPlanoSaude.getDescricao());
